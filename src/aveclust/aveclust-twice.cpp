...
#define UPPER_BOUND_NUM_EDGES 	120000000
...
float computeDist(TreeNode* nodeX, TreeNode* nodeY, float endLevel) {
	...
	if (nodeX->linkMap.find(nodeY->ID) != nodeX->linkMap.end()) {
		totalLinks = nodeX->numMembers * nodeY->numMembers;
		sumEdges = nodeX->linkMap[nodeY->ID].sumEdges;
		numUnlinked = totalLinks - nodeX->linkMap[nodeY->ID].numEdges;

		if (numUnlinked == 0 || allLoaded) {
			dist = (sumEdges + endLevel*numUnlinked) / totalLinks;
			if (dist < vExactDist[nodeX->ID]) {
				vExactDist[nodeX->ID] = dist;
				vExactNodes[nodeX->ID] = nodeY;
			}
						
			if (dist < vExactDist[nodeY->ID]) {
				vExactDist[nodeY->ID] = dist;
				vExactNodes[nodeY->ID] = nodeX;
			}	
		}		
		else {
			dist = (sumEdges + lambda*numUnlinked) / totalLinks;
			if (dist < vInexactDist[nodeX->ID]) 
				vInexactDist[nodeX->ID] = dist;
						
			if (dist < vInexactDist[nodeY->ID]) 
				vInexactDist[nodeY->ID] = dist;
		}		
	} 				
...
{   
	...
	for(mapIter = nodeY->linkMap.begin(); mapIter != nodeY->linkMap.end(); ++mapIter) {
		tempId = mapIter->first;
		if (parentNode->linkMap.find(tempId) == parentNode->linkMap.end()) {
		...
		// TEST THEORY	
		/*
		float dist1 = computeDist(nodeX, vActiveNodes[tempId], endLevel);		
		float dist2 = computeDist(nodeY, vActiveNodes[tempId], endLevel);
			
		if (parentNode->linkMap[tempId].numEdges < parentNode->numMembers * vActiveNodes[tempId]->numMembers)
		{
			float dist = computeDist(parentNode, vActiveNodes[tempId], endLevel);				
			if (dist < lambda/2 || fabs(dist - lambda/2) < EPSILON) 
			{				
				cout << parentNode->ID << "\t" << tempId << "\t" << dist1 << "\t" << dist2 << "\t" << dist;		
				getchar();			
			}
		}
		*/
	...
	vExactNodes[nodeX->ID] = vExactNodes[nodeY->ID] = 0;
	vExactDist[nodeX->ID] = vExactDist[nodeY->ID] = 1.0f;
	vInexactDist[nodeX->ID] = vInexactDist[nodeY->ID] = 1.0f;
	...
	for(mapIter = parentNode->linkMap.begin(); mapIter != parentNode->linkMap.end(); ++mapIter) {
		...
		if (vExactNodes[tempNode->ID] == nodeX || vExactNodes[tempNode->ID] == nodeY) {
...
{   
	...
	{				
		...
		{	
			maxNumEdges *= beta;
			cout << "new maxNumEdges: " << maxNumEdges << "\tnewId: " << newId << endl;
